...
6. The main application sends one authenticated JSON request per distinct accepted mode. The gateway serializes actual supplier calls and coalesces same-key requests, so queued work may extend calculation time rather than amplify supplier load. Route groups unfinished in the first pass receive one second-pass lookup before estimates are used, except that `PROVIDER_RATE_LIMITED` never receives an immediate same-mode retry. It strictly validates responses, maps facts to the participant only after the gateway response, deterministically orders valid route facts before scoring, and keeps per-mode failures as estimates. When the gateway returns a stable provider error code, such as `PROVIDER_RATE_LIMITED`, the estimated fallback row preserves that code in `failureReason` and persists it as `travel_options.failure_reason`; result cards surface the same reason beside the estimated source label. Result cards show real route details without rendering provider booking links; production enablement still depends on supplier route coverage/quota behavior and full user-flow acceptance.
