...
The local hardening migrations are `supabase/migrations/202607210001_publication_safety_and_run_recovery.sql` and `supabase/migrations/202607260001_atomic_materialization_and_policy_replay.sql`; the latter stays synchronized with `supabase/schema.sql`. Its forward cleanup preserves complete policy-valid unshared drafts, deletes incomplete or policy-invalid unshared trees, and fails only their active owning runs with a safe publication diagnostic while leaving shared/superseded history unchanged. Batch B was verified locally on PostgreSQL 17.10 with 5 files / 74 tests, root lint, 71 files / 457 tests, and a production build. Remote migration state was not queried or applied, and live supplier acceptance was not rerun; both remain explicit operator work. Gateway source files were untouched by Batch B.
