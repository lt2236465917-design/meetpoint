...
The approved mainland topology is defined by `deploy/aliyun/compose.yaml`: three private Compose services — loopback-bound Next.js `frontend`, unexposed `travel-gateway`, and unexposed `run-worker` — on a single mainland ECS instance. The filed canonical host is `www.meetpoint.space`, with the apex redirecting to it. Detailed commands and the Nginx template live in `deploy/aliyun/README.md`.

The ICP filing is approved and public DNS for `www` / apex / `media` is live. Compose binds the frontend to `127.0.0.1:3001` (not host `3000`, which may already be used by other sites on a shared ECS). `run-worker` advances nonterminal automatic and alternative recommendation runs in-process without browser POSTs; host confirmation of alternatives remains human-only. Live `/opt/meetpoint` runs a healthy Compose `run-worker` (no host port) on `node:20-slim` with an explicit service-role Supabase `ws` transport; leave-and-finish checklist #11 PASS (2026-07-28, plan `ZNM4ZK`). Host Nginx terminates HTTPS for `www.meetpoint.space`, redirects the apex, and proxies to that loopback port. Scenic MP4 files are on OSS bucket `meetpoint-media` and delivered through CDN hostname `media.meetpoint.space` with `NEXT_PUBLIC_SCENIC_BASE_URL=https://media.meetpoint.space`. Keep security-group application ingress limited to `80`/`443`. Fresh mainland-phone completed/incomplete/failed/retry flows passed on 2026-08-02. UptimeRobot Free checks the homepage and city-search API every five minutes with verified email notification; internal service-log/coverage routing, WeChat direct-open classification, and public-security filing remain operator work. Overseas Vercel continues to use browser-authenticated advance only and does not ship a run-worker in the current release.
...
The 2026-08-02 production acceptance exercised Chat, Responses, bounded shadow, a controlled Responses-primary switch, and the one-variable rollback. The final runtime deliberately remains `DEEPSEEK_TRANSPORT=chat_completions`, `DEEPSEEK_SHADOW_TRANSPORT=off`, and sample rate `0`. Any future switch is an environment-only operation that must inspect effective server-only configuration without printing secrets, repeat authenticated smoke, and retain immediate rollback to Chat.
...
| `RUN_WORKER_POLL_INTERVAL_MS` | `run-worker` only | Poll interval for the Compose recommendation worker; defaults to `3000` ms. |
| `RUN_WORKER_HEARTBEAT_PATH` | `run-worker` only | Heartbeat file path for Compose healthchecks; defaults to `/tmp/run-worker-heartbeat`. |
...
On China ECS Compose, the private `run-worker` service keeps automatic and alternative runs moving through the same lease-guarded `advanceRun` path without client POSTs (`RUN_WORKER_POLL_INTERVAL_MS`, default 3s). The authenticated advance route remains an optional accelerator when a participant tab is open and is the Vercel overseas fallback (no worker required there). If the worker is down long enough for the 2-hour inactivity window to elapse, stale expiry still surfaces `RUN_STALE_EXPIRED`. Alternative runs stop at `awaiting_host_confirmation` until host-token confirmation; the worker does not confirm them.
...
6. Confirm a new calculate request returns HTTP 202 `created`, while an identical active request returns HTTP 200 `resume_existing`; the advance endpoint reports the same run ID and persisted progress. On China ECS with a healthy `run-worker`, closing all browsers after start should still reach a terminal or `awaiting_host_confirmation` state. On `/p/[code]/result` with a pending automatic run, an open tab may still issue one authenticated bounded advance before refreshing as an optional accelerator (required path on Vercel without a worker).
7. Without Supabase and supplier-backed verified quotes, confirm the fallback run ends `incomplete` and exposes no shared result. Current production evidence and residuals: `docs/acceptance/2026-08-02-reliable-baseline-production-acceptance.md`.
...
- External homepage/API availability routing is accepted in `docs/acceptance/2026-08-08-external-availability-monitoring-acceptance.md`. It does not cover internal worker, supplier, database, or recommendation-integrity signals. Until those redacted alerts are wired, inspect recent `travel-gateway` and `run-worker` Compose logs, run the redacted contract canary, and query recent coverage. Never paste raw provider output, prompts, responses, participant data, or credentials into incident records.
