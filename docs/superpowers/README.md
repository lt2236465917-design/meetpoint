...
The Vercel Services release remains the overseas fallback (browser advance only; no run-worker in the current release). The primary China target is live on Alibaba Cloud mainland ECS at `www.meetpoint.space` (loopback frontend on host `3001`, private gateway, private `run-worker`, Nginx HTTPS, OSS/CDN scenic at `media.meetpoint.space`). Full create/join/plan/result mainland-phone acceptance — including leave-and-finish with the worker healthy — and public-security filing remain open before deleting release-safety worktrees/stashes. Live checklist and remote preflight: `docs/acceptance/2026-07-28-aliyun-mainland-phone-acceptance.md`. Canonical prior Vercel evidence: `docs/acceptance/2026-07-27-production-release-acceptance.md`.
