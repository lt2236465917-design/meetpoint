...
describe("HomePage", () => {
  it("renders a full-bleed zero-scroll hero with CTA and records entry", () => {
    ...
    expect(html).toContain("这次在哪儿见");
    expect(html).not.toContain("散在几座城的朋友，");
    expect(html).not.toContain("这次在哪儿见？");
    expect(html).toContain("发起见面计划");
    expect(html).toContain('href="/create"');
    expect(html).toContain("hero-cta");
    expect(html).not.toContain("hero-readability-zone");
    expect(html).toContain("readable-title");
    expect(html).toContain("train-bob");
    expect(html.match(/<video/g)).toHaveLength(4);
    expect(html.match(/<source/g)).toHaveLength(8);
    expect(html.match(/media="\(max-width: 767px\)"/g)).toHaveLength(4);
