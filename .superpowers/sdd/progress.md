...
- Current evidence: `docs/acceptance/2026-08-02-reliable-baseline-production-acceptance.md`. Remaining residuals at that cutoff were the WeChat direct-open ICP interstitial, external alert routing, and an unprocured second supplier. No push, credential rotation, worktree deletion, or stash deletion occurred.
...
- Release integration (2026-07-27): Repository Audit Batches A–C on `origin/main`; product `206cd7e` / evidence `b38e60c` pushed without force.
- Vercel overseas Ready; China audience requires filed Alibaba host (mainland `*.vercel.app` timed out historically).
- Mainland go-live (2026-07-28): `www.meetpoint.space` via Nginx → `127.0.0.1:3001`; private gateway + `run-worker`; scenic CDN `media.meetpoint.space`. Remote preflight and leave-and-finish #11 passed. Fresh full-flow mainland-phone acceptance followed on 2026-08-02; public-security filing remains operator compliance work.
- Repository cleanup (2026-08-08): local and remote now expose only `main` at `9b965a7`; 12 audited auxiliary worktrees and eight merged local branches were removed, and remote `codex/repository-audit-complete` was deleted. Four dirty worktrees were first preserved as named stashes; together with two existing duplicate safety stashes, six recoverable stashes remain. Do not drop or clear them until the WeChat direct-open residual and internal service-log/coverage alert routing are deliberately closed.
- Public read-only smoke (2026-08-08): `https://www.meetpoint.space/` returned the `meetpoint` brand and `京ICP备2026025115号-3`; `/api/cities/search?q=上海` returned the canonical Shanghai result. This does not close WeChat in-app classification or public-security filing.
- External availability monitoring (2026-08-08): UptimeRobot Free checks the canonical homepage and `/api/cities/search?q=北京` every five minutes. Both first checks were `Up`, the incident list was empty, and paired simulated `DOWN`/`UP` emails reached the verified operator destination. This closes public homepage/API availability alerting at zero recurring cost; internal redacted log and coverage/divergence alert routing remains open. Evidence: `docs/acceptance/2026-08-08-external-availability-monitoring-acceptance.md`.
