...
- Batch A operational boundary (2026-07-26): local migration `supabase/migrations/202607210001_publication_safety_and_run_recovery.sql` was not queried or applied remotely. Live FlyAI/supplier acceptance was not run, so fixture/build success is not live publication evidence.
...
- Batch B operational boundary (2026-07-26): remote Supabase migration state was not queried or applied, live FlyAI/supplier acceptance was not rerun, and no credential rotation is claimed. The local PostgreSQL URL and database output were not recorded in the repository.
- Batch C remains the next audit backlog: validate Amap adcode/name pairs and real calendar dates, finish application-side arrival-instant validation, keep private preview terminal failures visible to the requester, prevent private runs from replacing public automatic progress selection, and replace collision-prone plan-code creation with retry-safe generation. It has no approved standalone spec or implementation plan yet.
