...
- Task 7: implementation committed as `e14ccc1` (`feat: enforce exact recommendation policy`); focused 29/29, root 269/269, lint/build verified; independent spec review passed. The legacy `scoring.ts` production module remains intentionally unchanged until its callers migrate in Tasks 9–10 and the path is removed in Task 13; the spec reviewer accepted this sequencing. Task 7 is not complete because independent code-quality review has not run.
- Next: resume Task 7 at the code-quality review gate for range `0fed62a..e14ccc1`; fix and re-review any Critical/Important findings before marking Task 7 complete or starting Task 8.
