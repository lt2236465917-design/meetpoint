...
export function buildCalculationSystemPrompt(input: {
  ...
  return [
    ...
    "省钱方案必须选择每位参与者直达优先候选中的最低票价；已接受的普通火车、动车高铁和飞机一并按真实票价比较。省时方案团队票价不得超过省钱方案的 130%。",
    "城市排序依次为团队省钱总价、直达人数、票价公平差、团队时长、城市代码；不得引入权重。",
    "Calculation Agent 必须自行选择城市、构建 saving 和 fast 方案，并引用每位参与者的 quote_id。",
    "必须调用算术和证据校验；确定性策略校验会独立拒绝错误的城市、路线、总价或比较证据，但不会预先选择推荐结果。",
