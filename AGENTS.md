...
- Plan/result IA is StatusLane + glass schemes per `docs/superpowers/specs/2026-07-17-plan-result-ia-design.md` (shipped). Do not deep-restyle alternatives/manage; their approved fixed scenic background is shared-shell atmosphere only. Phase 4 `PeakScenicAccent` stays on wait/reveal peaks; route-fixed shell scenic provides page-level continuity and does not replace peak accents.
