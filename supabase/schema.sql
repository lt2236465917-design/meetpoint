...
create table if not exists participants (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint participants_departure_lat_valid
    check (departure_lat is null or departure_lat between -90 and 90),
  constraint participants_departure_lng_valid
...
create table if not exists recommendation_runs (
  ...
  error_summary text,
  baseline_city_code text,
  baseline_city_name text,
  baseline_policy_version text,
  baseline_evidence_level text,
  ...
  constraint recommendation_runs_baseline_complete check (
...
create table if not exists route_tasks (
  ...
  constraint route_tasks_query_priority_valid
...
begin
  ...
  if v_task_count = 0 or jsonb_array_length(p_priorities) <> v_task_count
    ...
    or exists (
      select 1 from jsonb_to_recordset(p_priorities)
